...
- **Story — How 30 Years Changed Football:**
  https://public.tableau.com/shared/NQM5JTT2F?:display_count=n&:origin=viz_share_link
...
| `processed_data/` | The analysis-ready CSV tables the Tableau workbook reads, plus `PROCESSING_LOG.txt` |
| `plots/` | Data-exploration charts rendered by the script (matplotlib) |
| `screenshots/` | Captures of the finished Tableau dashboards and story |
| `How 30 Years Changed Football - full project.twbx` | Packaged Tableau workbook — both dashboards and the story |
| `Final Report.pdf` | The submitted project report |
| `README.md` | This file |

Note the two image folders differ: `plots/` holds the exploratory matplotlib charts used while
getting to know the data, while `screenshots/` holds captures of the finished Tableau dashboards
and story. The contents of `processed_data/` are fully regenerated by running the script.
